        ...
        <div class="row m-bottom-25 justify-content-center">
            <div class="col-md-8 col-12">
                <div class="form-container">
                    <h4 class="fw-bold m-bottom-30 text-color-100 text-center">Update Idol Form</h4>
                    <form method="post" action="{{ route('idols.update', $idol->id) }}">
                        @method('put')
                        @csrf
                            ...
                            <select class="form-select @error('artist_id') is-invalid @enderror" aria-label="Select Artist"
                                ...
                                @foreach ($artists as $artist)
                                    <option value="{{ $artist->id }}"
                                        {{ old('artist_id', $idol->artist_id) == $artist->id ? ' selected' : ' ' }}>
                                        {{ $artist->artist_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="m-bottom-15">
                            ...
                            <input type="text" class="form-control @error('stage_name') is-invalid @enderror"
                                name="stage_name" id="stage_name" placeholder="Yoona" value="{{ old('stage_name', $idol->stage_name) }}">
                            @error('stage_name')
                        ...
                        <div class="m-bottom-15">
                            ...
                            <input type="text" class="form-control @error('birth_name') is-invalid @enderror"
                                name="birth_name" id="birth_name" value="{{ old('birth_name', $idol->birth_name) }}"
